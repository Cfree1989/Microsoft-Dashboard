...
| `varShowExportModal` type inconsistency | Export modal uses `Boolean`; all others use `Int` (record ID). Migrate to `0`/`ID` pattern for one consistent mental model and simpler "close all modals" helpers. | Gemini, Composer 2 | ~~Fixed Apr 22, 2026~~ — `varShowExportModal` now follows the app’s numeric modal convention: `App.OnStart` seeds it to `0`, the `Report` header button opens it with `1`, and both export-modal close targets reset it to `0`. `conExportModal.Visible` now uses `varShowExportModal <> 0`, matching the rest of the modal mental model without changing export behavior. **Pushed to live coauthoring session via `compile_canvas` + verification `sync_canvas`. Docs updated:** `PowerApps/StaffDashboard-App-Spec.md` modal state init, variable table, header button, and export modal visibility/close formulas. Compiler verified: warnings only, no blocking errors. |
