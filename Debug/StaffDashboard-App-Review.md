...
|---|-----------|---------|--------|-----------|
| 1 | `btnSchedSave.OnSelect` — scrSchedule | No `IfError` around `RemoveIf` or `Patch`. On failure: existing shifts are deleted but not re-created (data loss), `varSchedEditSaving` is stuck `true`, and no error is surfaced to the user. | All 4 | ✅ Open — `scrSchedule.pa.yaml` lines 345–431 |
...
| `btnSchedSave` — destructive `RemoveIf` with no confirmation | No guard on `varSchedSelectedEmail` being blank, no confirmation step before wiping all rows. Minimum: a blank check guard; ideally a confirm dialog. | Gemini, Composer 2 | ✅ Open — `scrSchedule.pa.yaml` line 346 |
