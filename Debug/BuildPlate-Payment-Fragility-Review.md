...
3. **Unique constraint on `Payments.TransactionNumber`?** — **Resolved (server-side).** `Flow-(H)-Payment-SaveSingle` and `Flow-(I)-Payment-SaveBatch` reject duplicate **`TransactionNumber`** values only for **`PaymentType = TigerCASH`** (delegable **`Get items`** on **`Payments`**). **Check** and **Code** / grant references may repeat; there is no SharePoint unique index on the column.
